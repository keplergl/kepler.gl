 ...
 */
type EffectFn = (resolve: ResolveFn, reject: RejectFn, ctx?: ProgressCtx) => any;
  ...
  const descriptor: TaskDescriptor = {
    ...
    chain(next) {
      return buildTask(
        payload,
        (runner, resolve, reject, ctx) =>
          runFn(runner, value => next(value).run(runner, resolve, reject, ctx), reject, ctx),
        `Chain(${label})`
      );
    }
  };
 ...
 */
function wrapWithInstrumentation(effect: EffectFn, payload: any, label: string): TaskDescriptor {
...
export function fromPromise(fn: (arg?: any) => Promise<any>, label: string): TaskFactory {
  return Object.assign(
    (arg?: any) =>
      wrapWithInstrumentation((resolve, reject) => fn(arg).then(resolve, reject), arg, label),
    {type: label, label}
  );
}

/**
 * Create a task factory for Promise-based effects that report progress.
 *
 * The task argument contains the effect argument and a mapper that turns each
 * progress value into an action. The middleware dispatches those actions.
 */
export function fromPromiseWithProgress(
  fn: (input: {arg: any; onProgress: (progress: any) => void}) => Promise<any>,
  label: string
): TaskFactory {
  return Object.assign(
    (payload?: {arg: any; onProgress?: (progress: any) => any}) =>
      wrapWithInstrumentation(
        (resolve, reject, ctx) => {
          // Without a dispatch context (test runners, simulateTask) progress is
          // dropped and the mapper is never invoked.
          const emitProgress = ctx
            ? (progress: any): void =>
                ctx.onProgress(payload?.onProgress ? payload.onProgress(progress) : progress)
            : (): void => undefined;
          return fn({arg: payload?.arg, onProgress: emitProgress}).then(resolve, reject);
        },
        payload,
        label
      ),
...
export function taskCreator(
  ...
  return Object.assign(
    (arg?: any) =>
      wrapWithInstrumentation((resolve, reject) => fn(arg, resolve, reject), arg, label),
    {type: label, label}
  );
}
...
export const allSettled = (tasks: TaskDescriptor[]): TaskDescriptor => combineParallel(tasks, true);
...
export const taskMiddleware =
  ...
  (action: any): any => {
    ...
    const dispatch = (nextAction: any) => microtask.then(() => store.dispatch(nextAction));
...
export const simulateTask = (task: TaskDescriptor, runner: RunnerFn): any => runSync(task, runner);
...
export const succeedTaskWithValues = (task: TaskDescriptor, values: any[]): any => {
  ...
  return runSync(task, (_effect, resolve) => {
    if (cursor >= values.length) {
      throw new Error(`[kepler.gl] succeedTaskWithValues: ran out of values at index ${cursor}`);
    }
