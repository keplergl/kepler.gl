...
- Kepler example app: ../../examples/sqlrooms
...
locale policy are separate from the config schemas; this migration does not
claim a full replacement for the existing Kepler application.
...
The integration example uses local Kepler sources and published SQLRooms
packages, so no sibling SQLRooms checkout is required.
