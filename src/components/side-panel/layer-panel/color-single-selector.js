...
export default class ColorSingleSelect extends ReactBaseComponent {
  ...
  render() {
    ...
    return (
      <Accordion className="one-whole flush color-single-selector"
        style={{width, margin: 'auto'}}>
        <div className="display--inline-block color--block"
  ...
  return (
    <div>
      ...
      {columns.map((theme, col) => (
        <g transform={`translate(${scale(col) - bandWidth / 2 + 4 + PADDING}, 0)`}
           key={theme}>
          {range(1, rows + 1, 1).map((key, i) =>
            <rect className="cursor--pointer"
                  ...
                  style={{
                    ...
                    stroke: selectedColor === ColorsByTheme[theme][String(key)] ? '#ff0000' : ColorsByTheme[theme][String(key)],
                    strokeWidth: 2
                  }}
                  onClick={e => onSelectColor(ColorsByTheme[theme][String(key)], e)}>
            </rect>
          )}
        </g>
      ))}
