...
export default class LayerConfigurator extends Component {
  ...
  _renderScatterplotLayerConfig({layer, visConfiguratorProps, layerChannelConfigProps, layerConfiguratorProps}) {
    return (
      <div className="push-small--top">
        ...
        {layer.config.colorField ?
          <ColorRangeConfig {...visConfiguratorProps} /> : null}
        <VisConfigSlider
  ...
  _renderClusterLayerConfig({layer, visConfiguratorProps, layerConfiguratorProps, layerChannelConfigProps}) {
  ...
  _renderAggregationLayerConfig({layer, visConfiguratorProps, layerConfiguratorProps, layerChannelConfigProps}) {
    ...
    return (
      <div className="push-small--top">
        ...
        <VisConfigSlider
          ...
          {...visConfiguratorProps}
          label={`${type === LAYER_TYPES.grid ? 'Grid Size' : 'Hexagon Radius'} (km)`}/>
        <VisConfigSlider
        ...
        <VisConfigSwitch
          ...
          {...visConfiguratorProps}/>
        {enable3d ? <ChannelByValueSelector
          {...layerChannelConfigProps}
          channel={layer.visualChannels.size}
  ...
  _renderHexagonIdLayerConfig({layer, visConfiguratorProps, layerConfiguratorProps, layerChannelConfigProps}) {
  ...
  _renderLineLayerConfig({layer, visConfiguratorProps, layerConfiguratorProps, layerChannelConfigProps}) {
  ...
  _renderGeojsonLayerConfig({layer, visConfiguratorProps, layerConfiguratorProps, layerChannelConfigProps}) {
    const {
      meta: {featureTypes = {}},
      config: {visConfig}
    } = layer;
    ...
    return (
      <div className="push-small--top">
        ...
        {featureTypes.polygon ?
          <VisConfigSwitch
            {...visConfiguratorProps}
            {...LAYER_VIS_CONFIGS.filled}/> : null}
        <ChannelByValueSelector
          channel={layer.visualChannels.color}
          {...layerChannelConfigProps}/>
        {layer.config.colorField ? <ColorRangeConfig {...visConfiguratorProps}/> : null}

        {
          /* Stroke Width */
        }
        {featureTypes.polygon ? <VisConfigSwitch
          {...visConfiguratorProps}
          {...LAYER_VIS_CONFIGS.stroked}/> : null}
        ...
        {featureTypes.polygon && visConfig.filled ?
          <div>
            ...
            />
            {visConfig.enable3d ? <ChannelByValueSelector
  ...
  render() {
    const {layer, datasets, panelWidth, isAdding, updateLayerConfig} = this.props;
    ...
    return (
      <div className="soft-tiny layer-panel__config">
        {Object.keys(datasets).length > 1 && <SourceDataSelector
          datasets={datasets}
          id={layer.id}
...
export const VisConfigSlider = ({layer: {config}, property, label, range, step, isRanged, disabled, onChange}) => (
  <SidePanelSection disabled={Boolean(disabled)}>
    ...
    <RangeSlider
      ...
      onChange={value => onChange(
        {[property]: isRanged ? value : value[1]})}/>
  </SidePanelSection>
);
...
const LayerColorSelector = ({layer, panelWidth, onChange, label}) => (
  <SidePanelSection disabled={layer.config.colorField}>
    ...
    <ColorSingleSelector
      ...
      selectedColor={rgb(...layer.config.color).toString().toUpperCase()}/>
  </SidePanelSection>
);
...
const VisColorSelector = ({layer, panelWidth, onChange, label, property}) => (
  <SidePanelSection
    ...
    <ColorSingleSelector
      ...
      selectedColor={
        rgb(...(layer.config.visConfig[property] || layer.config.color)).toString().toUpperCase()}/>
  </SidePanelSection>
);
...
const ChannelByValueSelector = ({layer, channel, panelWidth, onChange, fields, description}) => {
  const {
    channelScaleType,
    domain,
    field,
  ...
  const supportedFields = fields.filter(({type}) => supportedFieldTypes.includes(type));
  const selectedField = layer.config[field];
  const scaleOptions = selectedField && FIELD_OPTS[selectedField.type].scale[channelScaleType] || [];
...
const AggrColorScaleSelector = ({layer: {config}, onChange}) => (
  <DimensionScaleSelector
    label="Color Scale"
    options={config.colorField ? FIELD_OPTS[config.colorField.type].scale.colorAggr :
      FIELD_OPTS.integer.scale.colorAggr}
    scaleType={config.colorScale}
    onSelect={val => onChange({colorScale: val}, 'color')}/>
);

const AggregationTypeSelector = ({layer: {config: {visConfig}}, field, property, options, onChange}) => (
  <SidePanelSection>
