...
const Nav = ({items, selectedIndex, onClick}: NavProps) => (
  <NavContainer>
...
class Showcase extends PureComponent<Record<string, never>, ShowcaseState> {
  state = {
    selectedIndex: 3
  };
      ...
      <div>
        <CarouselContainer>
          <Carousel
            ...
            onChange={(i: number) => this.setState({selectedIndex: i})}
          >
        ...
        <Nav
          ...
          selectedIndex={this.state.selectedIndex}
          onClick={(i: number) => this.setState({selectedIndex: i})}
        />
      </div>
    );
  }
