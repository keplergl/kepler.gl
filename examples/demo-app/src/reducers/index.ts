 ...
 */
export const loadRemoteResourceSuccess = (state: any, action: any) => {
  ...
  let processorMethod: any = processRowObject;
...
const loadRemoteDatasetProcessedSuccess = (state: any, action: any) => {
  const {config, datasets, options} = action.payload;

  const parsedConfig: any = config ? KeplerGlSchema.parseSavedConfig(config) : undefined;
...
export const loadRemoteResourceError = (state: any, action: any) => {
