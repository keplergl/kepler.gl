...
interface LoadRemoteMapError {
  message: string;
}

const Error = ({error, url}: {error: LoadRemoteMapError; url?: string}) => (
  <StyledError>
...
interface LoadRemoteMapProps {
  onLoadRemoteMap: (options: {dataUrl: string}) => void;
  error?: LoadRemoteMapError | null;
  option?: {dataUrl?: string};
}

interface LoadRemoteMapState {
  dataUrl: string;
  error: LoadRemoteMapError | null;
  submitted: boolean;
}

class LoadRemoteMap extends Component<LoadRemoteMapProps, LoadRemoteMapState> {
  state: LoadRemoteMapState = {
    dataUrl: '',
    error: null,
    submitted: false
  };

  onMapUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    this.setState({
