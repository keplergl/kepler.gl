...
const CustomLoadDataModalFactory = (...deps: any[]) => {
  const LoadDataModal = (LoadDataModalFactory as (...args: any[]) => any)(...deps);
  ...
  const loadingMethods = [
    defaultLoadingMethods.find((lm: any) => lm.id === 'upload'),
    defaultLoadingMethods.find((lm: any) => lm.id === 'tileset'),
    additionalMethods.remote,
    defaultLoadingMethods.find((lm: any) => lm.id === 'storage'),
    additionalMethods.sample
  ];

  return withState(
    [],
    (state: any) => ({...state.demo.app, ...state.demo.keplerGl.map.uiState, loadingMethods}),
    {
