...
- `standardize` — `{ datasetName, variableName, method: deviationFromMean|standardizeMAD|rangeAdjust|rangeStandardize|standardize, outputDatasetName }` → `{ outputDatasetName, outputVariableName }`.
