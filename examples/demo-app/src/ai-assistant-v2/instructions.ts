...
- For kepler.gl tools:
  ...
  2. Do not call addLayer tool after loadData tool or saveToolResults tool, as these tools automatically create a map layer.
  3. For addLayer tool: if dataset can not be found, please prompt the user to save the dataset first
  4. Do not call saveToolResults after tableTool, as tableTool will automatically save the dataset to kepler.gl
...
- For dataClassify tool: when classify data into bins using break values, please use the following rules:
    ...
    b. For example, for breaks at [1000, 1100], the classification or bins should be:
      - b1: values < 1000
      - b2: 1000 <= values < 1100
      - b3: values >= 1100
...
- Colocation is a map that shows the co-location of two variables V1 and V2 from a dataset A
  1. create a categorical variable for the first variable by other tools, e.g.
    ...
    b. clusters in lisa
  2. create a categorical variable for the second variable using the same tool
